        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                @foreach($beritaSekolah as $berita)
                    <a href="{{ route('beritasekolah.show', $berita->id) }}" class="p-6 border rounded-md hover:bg-gray-100 flex">
                        @if($berita->gambar)
                            <div class="mr-4">
                                <img width="100px" src="{{ asset('images/beritasekolah/' . $berita->gambar) }}" alt="{{ $berita->judul }}" class="object-cover rounded-md">
                            </div>&nbsp;&nbsp;
                        @endif
                        <div>
